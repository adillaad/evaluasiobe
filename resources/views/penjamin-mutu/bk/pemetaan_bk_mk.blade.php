        ...
        <div id="matrix-edit-banner" class="alert alert-info border-0 shadow-sm rounded-4 mb-4 p-3 d-none align-items-center justify-content-between">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    ...
                    <small class="text-secondary">Klik sel mana saja pada tabel di bawah untuk mencentang atau menghapus centang pemetaan Bahan Kajian (BK) ke Mata Kuliah.</small>
                </div>
            </div>
            <div class="d-flex align-items-center gap-2">
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
            <div class="card-body p-4">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-3">
                    <div>
                        <h4 class="card-title fw-bold mb-1">List Pemetaan BK - MK</h4>
                        <p class="text-muted small mb-0">Matriks keterhubungan antara Bahan Kajian (BK) dan Mata Kuliah.</p>
                    </div>
                    ...
                    @if (in_array($userOtoritas, ['Penjamin Mutu Program Studi', 'Kepala Program Studi', 'Penjamin Mutu Universitas', 'Penjamin Mutu Fakultas']))
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <a href="{{ route($currentPrefix . 'bk.bk-mk-add') }}" class="btn btn-primary rounded-3 font-weight-bold d-inline-flex align-items-center gap-1 px-3 py-2 shadow-sm">
                                <i class="mdi mdi-plus-circle font-18"></i>
                                <span>Tambah BK - MK</span>
                            </a>
                        </div>
                    @endif
                </div>

                <form id="form-matrix-edit" action="{{ route($currentPrefix . 'bk.bk-mk-matrix-update') }}" method="POST">
                    @csrf
                        ...
                        <table class="table table-hover align-middle border-light" id="matrix-table">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    @foreach ($bks as $bk)
                                        <th class="py-3 px-2 text-center" style="min-width: 85px;" title="{{ $bk->kode }}: {{ $bk->nama }}">
                                            <span class="badge bg-primary-soft text-primary font-12 px-2 py-1 rounded-2">{{ $bk->kode }}</span>
                                        </th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($mks as $index => $mk)
                                    <tr>
                                        ...
                                        @foreach ($bks as $bk)
                                            @php
                                                $isChecked = $mk->bk->contains('id', $bk->id);
                                            @endphp
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ 5 + count($bks) }}" class="text-center py-4 text-muted">
                                            <i class="mdi mdi-file-table-outline font-32 d-block mb-1"></i>
                                            Belum ada data pemetaan Bahan Kajian ke Mata Kuliah.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="scrollable-descriptions pe-2" style="max-height: 400px; overflow-y: auto;">
                    @foreach ($mks as $mk)
                        <div class="mb-4 p-3 bg-light rounded-3 border-light">
                            ...
                            <div class="font-13">
                                <strong class="text-dark d-block mb-1">Bahan Kajian (BK) Terkait:</strong>
                                @if ($mk->bk->isNotEmpty())
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach ($mk->bk as $bk)
                                            <span class="badge bg-white text-dark border px-2 py-1.5 rounded-2 shadow-xs">
                                                <strong class="text-primary">{{ $bk->kode }}:</strong> {{ $bk->nama }}
                                            </span>
                                        @endforeach
                                    </div>
                                @else
                                    <span class="text-muted fst-italic">Belum ada Bahan Kajian terkait.</span>
                                @endif
                            </div>
                        </div>
                    @endforeach
