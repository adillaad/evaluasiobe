                                        ...
                                        <div class="form-group row">
                                            <div class="col-5">
                                                <div class="form-group">
                                                    ...
                                                    @enderror
                                                    <input type="text" name="npm" class="visually-hidden"
                                                        value="">
                                                    <input type="text" name="nama" class="visually-hidden"
                                                        value="">
                                                    <input type="text" name="angkatan" class="visually-hidden"
                                                        value="">
                                                    <input type="text" name="prodi" class="visually-hidden"
                                                        value="">
                                                    <input type="text" name="universitasImg" class="visually-hidden"
                                                        value="">
                                                    <input type="text" name="universitasCPMK" class="visually-hidden"
                                                        value="">
                                    ...
                                    <div class="card mt-3">
                                        <div class="card-body">
                                            ...
                                            <div class="table-responsive overflow-auto"
                                                ...
                                                @if (auth()->user()->prodi->is_aptikom)
                                                    <table id="hasilProfesiTableAptikom" class="table table-bordered">
                                                        <thead>
                                                            <tr>
                                                                <th>No</th>
                                                                <th>Profesi</th>
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            $('#universitas').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route($currentPrefix . 'visualisasi.getAngkatanByUniversitas') }}", // route untuk kirim ke kontroler
                    method: 'GET',
                    data: {
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#selectPemetaanCpl').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route($currentPrefix . 'visualisasi.getPemetaanCpl') }}", // route untuk kirim ke kontroler
                    method: 'GET',
                    data: {
                        idCpl: idCpl,
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#angkatanForm').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route($currentPrefix . 'visualisasi.getNpmByAngkatan') }}", // route untuk kirim ke kontroler
                    method: 'GET',
                    data: {
                        angkatan: angkatan,
                        universitas: universitas
            ...
            $('#hasilVisual').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var hasilFinalProfesi = response.result.hasilFinalProfesi;
                        var chartDataProfesi = response.result.chartDataProfesi;
                        var courseArray = response.result.courseArray;
                        var courseCpmkRestrict = response.result.courseCpmkRestrict;
                        var isAptikom = response.result.is_aptikom;

                        // SUMMARY
                        console.log(hasilFinalProfesi);
                        console.log(chartDataProfesi);
                        
                        //dilakukan filter jika prodi merupakan aptikom, maka menggunakan profesi-cpmk, jika tidak maka menggunakan profil lulusan-cpl
                        // Hasil profil
                        if (isAptikom) {
                            $('#hasilProfesiTableAptikom tbody').empty();
                            // Iterasi hasilFinalProfesi
                            var nomor = 1;
                            $.each(hasilFinalProfesi, function(key, profesi) {
                                var totalRows = profesi.CPMKs.length;
                                var row = '<tr>';
                                row += '<td rowspan="' + totalRows + '">' + nomor + '</td>';
                                row += '<td rowspan="' + totalRows + '">' + profesi
                                    .nama + '</td>';

                                $.each(profesi.CPMKs, function(index,cpmk) {

                                    row += '<td>' + cpmk.CPMK + '</td>';
                                    row += '<td>' + cpmk.deskripsi +
                                    '</td>'; 
                                    row += '<td>' + cpmk.HasilCPMK + '</td>';
                                    if (index === 0) {
                                        row += '<td rowspan="' + totalRows + '">' +
                                            profesi.TotalAkhir.toFixed(3) + '</td>';
                                    }
                                ...
                                nomor++;
                                $('#hasilProfesiTableAptikom tbody').append(row);
                            });

                            // Karir chart
                            // Mendapatkan label dan data dari chartData
                            var labels = chartDataProfesi.map(function(profesi) {
                                return profesi.label;
                            });

                            var data = chartDataProfesi.map(function(profesi) {
                                return profesi.data;
                            });
                            var backgroundColors = chartDataProfesi.map(function(_, index) {
                        ...
                        for (var i = 0; i < soalTerendah.length; i++) {
                            ...
                            if (soalTerendah[i].idSoal) {
                                let baseUrl =
                                    "{{ route($currentPrefix . 'cetakSoal', ['id' => '__ID__']) }}";
                                baseUrl = baseUrl.replace('__ID__', soalTerendah[i].idSoal);

                                rowHtml += '<td><a href="' + baseUrl + '" target="_blank">' +
                        ...
                        document.getElementById("btnPrintPdf").addEventListener("click",
                            function() {
                                ...
                                fetch("{{ route($currentPrefix . 'visualisasi.generate-pdfVisualMahasiswa') }}", {
                                        method: "POST",
                                        headers: {
