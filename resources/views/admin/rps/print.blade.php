...
<table>
    ...
    @php
        ...
        $nama_eng  = optional($mkFound)->nama_eng ?? null;
        $nama_display = $nama_eng
            ? trim($nama) . ' (' . trim($nama_eng) . ')'
            : $nama;
        $nama = optional($mkFound)->nama ?? '-';
    ...
    <tr class="contain">
        <td class="sub-contain center-text">{{ $nama_display }}</td>
    ...
    <tbody>
        ...
        @forelse ($rpsActivities as $activity)
            <tr class="contain" @if(in_array((int)$activity->minggu, [8,16])) style="background-color: #ffff00;" @endif>
                ...
                <td class="sub-contain" style="vertical-align: middle; white-space: normal; word-break: break-word;">
                    @php
