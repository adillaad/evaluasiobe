...
class VisualisasiController extends Controller
{
    public function index()
    {
        ...
        return view('dosen.visualisasi.indexVisualisasi', compact('universitas', 'prodi'));
    }
    ...
    public function hasilVisualMahasiswa(Request $request)
    {
        ...
        $cplPerSoalWithKode = DB::select($query, array_merge(['npm1' => $npm,'npm2' => $npm,'npm3' => $npm,'npm4' => $npm], $bindings));
    ...
    public function hasilVisualCpmkMahasiswa(Request $request)
    {
        ...
        $query = "
                SELECT m.cpmk, SUM((m.BobotSoal * m.nilaiSoal) / q1.BobotJenisCPMK * m.examWeight / q2.sumExamWeight) AS r3, cpmks.kode
                FROM mutus m
                INNER JOIN (
                    SELECT Course, 
        ...
        $cpmkPerSoalWithKode = DB::select($query, ['npm1' => $npm,'course1' => $course,'npm2' => $npm,'course2' => $course,'npm3' => $npm,'course3' => $course,'npm4' => $npm,'course4' => $course]);
    ...
    public function indexAngkatan()
    {
        ...
        return view('dosen.visualisasi.indexVisualisasiAngkatan', compact('universitas', 'prodi'));
    }
    ...
    public function hasilVisualMahasiswaAngkatan(Request $request)
    {
        ...
        ";
            ...
            $cplResults = DB::select($query, ['npm1' => $npm,'npm2' => $npm,'npm3' => $npm,'npm4' => $npm,'id_prodi' => $prodiId,]);
    ...
    public function hasilVisualCpmkAngkatan(Request $request)
    {
        ...
        $queryCpmkAngkatan = "
                ...
                FROM (
                    ...
                    JOIN 
                        (SELECT 
                            NPM, MAX(CONCAT(SUBSTRING_INDEX(tahun, ' ', -1), CASE WHEN SUBSTRING_INDEX(tahun, ' ', 1) = 'Genap' THEN '1' ELSE '0' END)) AS max_year_semester
                        FROM 
                            mutus
                        WHERE 
                    ...
                    JOIN 
                        (SELECT 
                            NPM, Cpmk, Jenis, tahun, SUM(BobotSoal) AS BobotJenisCPMK 
                        FROM 
                            mutus 
                        WHERE 
                    ...
                    JOIN 
                        (SELECT 
                            NPM, tahun, Cpmk, SUM(examWeight) AS sumExamWeight 
                        FROM 
                            (SELECT DISTINCT 
                                NPM, tahun, Cpmk, Jenis, examWeight 
                            FROM 
                                mutus 
                            WHERE  
    ...
    public function indexMataKuliah()
    {
        ...
        $prodi = $prodiQuery->get();
        return view('dosen.visualisasi.indexVisualisasiMataKuliah', compact('universitas', 'prodi'));
    }
